...
// Dynamically imported inside handleFile to avoid SSR/bundling crashes
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        // Ensure we have a valid file type; fallback to image/jpeg if missing
        const contentType = compressed.type || file.type || 'image/jpeg';

        const supabase = createClient();
        const fileName = `${Date.now()}-${Math.random().toString(36).slice(2)}.${compressed.name.split('.').pop()}`;
