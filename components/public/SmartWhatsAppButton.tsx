...
function isProductPage(pathname: string): boolean {
  // Matches /product/slug, /fr/product/slug, /en/product/slug, /ar/product/slug
...
export function SmartWhatsAppButton({ whatsappNumber, defaultMessages }: SmartWhatsAppButtonProps) {
  const href = useMemo(() => {
    ...
    if (isProductPage(pathname)) {
      // Try to read product data from the hidden div injected on product pages
      ...
      if (dataEl) {
        ...
        const message = buildProductMessage(locale, title, price, currency, url);
        return getWhatsAppHref(whatsappNumber, message);
      }
    }

    // Default message for non-product pages
    const message = buildDefaultMessage(locale, defaultMessages);
    return getWhatsAppHref(whatsappNumber, message);
